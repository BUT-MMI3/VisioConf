...
class PassForgotten {
    constructor(controller) {
        ...
        this.instanceName = "PassForgotten";
        ...
        if (this.verbose || this.controller.verboseall) console.log("INFO (" + this.instanceName + ") - Création de l'instance PassForgotten : " + this.instanceName);
    ...
    handlePassForgotten = async (msg) => {
        ...
        try {
            ...
            if (user) {
                ...
                user.set({
                    "user_tokens.invitation": token,
                });
                await user.save();

                // Créer un lien vers la page d'inscription
                const resetLink = `http://localhost:3000/inscription/${token}`;
                ...
                if (this.verbose || this.controller.verboseall) console.log(`INFO (${this.instanceName}) - Email de réinitialisation de mot de passe envoyé à ${user.user_email} - Token : ${token}`);

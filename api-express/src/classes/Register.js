...
class Register {
    constructor(controller) {
        ...
        if (this.verbose || this.controller.verboseall) console.log("INFO (Register) - Création de l'instance Register : " + this.instanceName);

        this.controller.subscribe(this, this.listeMessagesEmis, this.listeMessagesRecus);
    }

    handleRegistration = async (msg) => {
        if (this.verbose || this.controller.verboseall) console.log("INFO (Register) - Traitement de la demande d'inscription");
        ...
        try {
            ...
            let user = await User.findOne({ "user_tokens.inscription": token});
            jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => err ? user = null : null)
            if (user) {
    ...
    traitementMessage = async (msg) => {
        if (this.verbose || this.controller.verboseall) console.log("INFO (Register) - Traitement du message : " + JSON.stringify(msg));
